...
100.times do |x|
  HostListing.create!(user_id: User.all[rand(0..9)].id, dodo_code: "dodo-#{x}", item_id: Item.first.id, selling: [true, false].sample, amount: 500, start_date: DateTime.now - 6.hours, end_date: DateTime.now + 6.hours, max_users: 200, allowed_users: 4)
end
